...
'''def main():
    ...
    train_input, batch_train = create_input_ops(dataset_train, 64,
                                           is_training=True)
    test_input, batch_test = create_input_ops(dataset_test, 64,
                                          is_training=False)
    sess=tf.Session()
    batch1 = sess.run(train_input,batch_train)
    batch2 = sess.run(test_input,batch_test)
    print batch1
    print batch2
