...
def optimize_loss(loss,
  ...
  with vs.variable_scope(name, "OptimizeLoss", [loss, global_step]):
    ...
    pprint(gradients_original)

    gradients=adjust_gradients(gradients,'1')
...
def adjust_gradients(grads_and_vars,lay):
    adjusted_gradients=[]
    for grad, var in grads_and_vars:
        if (grad is not None and (var.name =='Discriminator/d_'+lay+'_conv/w:0')):
            print(var.name)
        #if (grad is not None and(var.name.startswith('Discriminator_d_'+lay+'_conv/w'))):
            grad_shape=grad.get_shape()
            noise = tf.random_uniform([int(grad_shape[0]),int(grad_shape[1]),
                    int(grad_shape[2]),int(grad_shape[3])], minval=-1, maxval=1, dtype=tf.float32)
            print("=======this is noise========")
            pprint(noise)
            grad=grad+noise
        adjusted_gradients.append((grad,var))
    print("=======adjusted gradient========")
    pprint(adjusted_gradients)
